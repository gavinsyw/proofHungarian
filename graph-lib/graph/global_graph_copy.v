...
Module GlobalGraphCopy.

Section GlobalGraphCopy.
...
Definition weak_ecopy1 e (p1 p2: Graph * Graph') :=
  let (g1, g1') := p1 in
  let (g2, g2') := p2 in
  let P_ne := Union _ (Complement _ (eq e)) (fun ee => evalid g1' (emap g1 ee)) in (* TODO: redefine it with respectful_set *)
  g1 ~=~ g2 /\
...
Lemma triple9_copy: forall (g g1 g2: Graph) (g' g1' g2': Graph') (P: V -> Prop) root es es_done e0 es_later,
  ...
  pregraph_join
   ...
   (image_set (emap g1) (Union _ PE_sr PE1)) g' g1') ->
  let P_rec := Intersection _ P0 (Complement _ PV1) in
  let PV0 := reachable_by g (dst g e0) P_rec in
  let PE0 := Intersection _ (weak_edge_prop PV0 g) (evalid g) in
  copy P_rec (dst g e0) (g1, g1') (g2, g2') ->
  ...
  pregraph_join
   ...
   (image_set (emap g2) (Union _ PE_sr PE2)) g' g2'.
Proof.
  intros until es_later.
...
Proof.
  ...
  + apply edge_union_guarded_si in H0.
    ...
    subst e'.
    apply H.
    generalize e, H3. eapply triple1_aux1_ecopy; eauto.
...
Proof.
  ...
  assert (e0 <> e) by (generalize e, H3; eapply triple1_aux1_ecopy; eauto).
  rewrite H5 in H6 |- *.
...
Lemma triple9_ecopy1: forall (g g2 g3: Graph) (g' g2' g3': Graph') (P: V -> Prop) root es es_done e0 es_later,
  ...
  exists PE_sr,
  Included PE_sr PE_root /\
  Included PE3_root PE_sr /\
  pregraph_join
   (image_set (vmap g3) (Union _ (eq root) PV2))
   (image_set (emap g3) (Union _ PE_sr PE2)) g' g3'.
Proof.
  intros until es_later.
  intros H_VVALID H_P P0 H_OUT_EDGES H_NODUP H_ES PV2 PE2 PE2_root PE_root
         [PRE_si [PE_sr [Hsr0 [Hsr1 PRE_pj]]]] ECOPY PE3_root.
  destruct ECOPY as [ECOPY_si [ECOPY_prv [ECOPY_gpre
                     [? [? [? ?]]]]]].
  exists (Union _ PE_sr (eq e0)).
  split; [| split].
  Focus 1. {
    apply Union_Included; split; auto.
    unfold Included, Ensembles.In; intros ee ?; subst ee.
    unfold PE_root; rewrite H_ES, in_app_iff.
    simpl; auto.
  } Unfocus.
  Focus 1. {
    unfold Included, Ensembles.In; intros e ?.
    unfold PE3_root in H3.
    rewrite in_app_iff in H3; simpl in H3.
    rewrite Union_spec.
    unfold Included, Ensembles.In, PE2_root in Hsr1.
    specialize (Hsr1 e).
    tauto.
  } Unfocus.
  assert
   (Same_set
     (image_set (emap g3) (Union E (Union E PE_sr (eq e0)) PE2))
     (Union _ (image_set (emap g3) (Union E PE_sr PE2)) (eq (emap g3 e0)))).
  Focus 1. {
  ...
  rewrite H3; clear H3.
  apply pregraph_join_edge_union with (G2 := g2'); auto.
