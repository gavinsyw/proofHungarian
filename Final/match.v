...
Proof.
  ...
  auto.
Qed.
Hypothesis No_dup : forall e, cross_path e -> NoDup e.
...
Proof.
  ...
  assert (In a p).
  simpl in H0.
  pose proof classic (E_eqb a0 a1 = true).
  destruct H1.
  rewrite H1 in H0.
  apply IHp in H0. exact H0.
...
Lemma list_sub_pre : forall a0 a m p , 
   ~ In a p -> 
   length (list_sub_e (a0 :: a :: m) p) =
      S (length (list_sub_e (a0 :: m) p)).
Proof.
  intros.
  Admitted.

Lemma list_sub_pre_in : forall a0 a1 a  m p , 
   length (list_sub_e (a0 :: m) p) =
     S (length (list_sub_e (a0 :: remove_e a m) p)) -> 
      length (list_sub_e (a0 :: a1 :: m) p) =
            S (length (list_sub_e (a0 :: a1 :: remove_e a m) p)).
Proof.
  intros.
  Admitted.


Theorem  remove_e_cons : forall a a1 m , a<>a1 ->
           remove_e a (a1::m) =a1:: remove_e a m.
Proof.
 intros.
 simpl. remember (E_eqb a a1) as s.
 destruct s. symmetry in  Heqs. rewrite E_eqb_eq in Heqs.
 unfold not in H. apply H in Heqs. inversion Heqs.
 auto.
Qed.
...
Proof.
  ...
  - inversion H.  destruct H0. subst.
    + simpl.  rewrite E_eqb_relf.
      rewrite (list_remove_notIn a m H5).  apply list_sub_pre. auto.
    +
     apply  not_in_app  in H1. destruct H1.
      pose proof (IHm H6 H0 H7). subst.
      assert (a<>a1). unfold not. intros. inversion H. subst.
      unfold not in H5. apply H5. auto.
      rewrite (remove_e_cons a a1 m H3).
      apply list_sub_pre_in. auto.
    intros. remember (E_eqb x0 y) as h. destruct h. left.
    apply E_eqb_eq. symmetry. auto.
    right. unfold not. intros. subst.
    rewrite E_eqb_relf in Heqh. inversion Heqh.
Qed.
...
Proof.
  ...
  - pose proof (NoDup_Add).
    ...
    rewrite H6.
    clear IHm H2 H3 H6.
    pose proof (list_sub_right_In_stay a a0 p m  H1 H H4 H0).
    auto.
Qed.
...
Proof.
  ...
  - destruct p.
    ...
    + destruct H. assert (1<=length(e::p)). simpl. omega.
      ...
      assert (h1: NoDup (e::p)).
      apply ( NoDup_cons_1 E a (e::p) h) .
      pose proof (IHp H2 H3 H4 h1). clear H0 IHp .
      destruct H5.
       ...
       * left. destruct H0. rewrite  (even_cons a (e::p)). rewrite H0. split.
         ...
         pose proof (list_sub_length_cons_In a (e::p) M H6).
         inversion h.
         pose proof (H7 H10 X).
         rewrite H12 in H5.
...
Theorem aug_path_odd : forall p,
  NoDup M -> aug_path_pre p -> 1 <=length (p) -> ~ In (last p de) M ->  Nat.even (length p) = false.
Proof.
  intros p X.
  intros.
  destruct p.
...
Proof.
  ...
  pose proof (aug_path_odd p X H H0 H3).
  pose proof (NotSmall p X H1 H0 H3).
...
(*定理2：通过一个更大匹配一定能找到一条增广路径*)

Lemma disjoint_edges_cons : forall (e  :E) m , 
      disjoint_edges_set BG (e::m) -> disjoint_edges_set BG m.
Proof.
  intros.
  unfold disjoint_edges_set in H.
  unfold disjoint_edges_set; intros.
  specialize (H e1 e2).
  destruct H.
  ...
  - exact H2.
  - exact H3.
  - unfold disjoint_edges.
    split.
    exact H.
    exact H4.
Qed.

Fixpoint to_nodire_l (e :list E) (v : list V): list V :=
match e with 
|nil => nil
|n::l => if Inb (fst n) v then
            (if Inb (snd n) v then to_nodire_l l v else 
               (snd n)::(to_nodire_l l ((snd n)::v)) )
         else 
         (if Inb (snd n) v then (fst n)::(to_nodire_l l v) else 
              (fst n) ::((snd n)::(to_nodire_l l ((snd n)::v))) 
         )
end.

Definition to_no_l (e : list E) := to_nodire_l e nil.
Print Ma.
Definition Madefult (u: option V): V :=
match  u with
|None => O
|Some w => w
end.
Fixpoint to_nod_f (e : list E)(s : list V) (v : list V) : list E :=
 match v with
|nil => nil
|u::l=> let w:= Madefult (Ma u e ) in 
           if Inb u  s then (to_nod_f e s l)
            else 
          (u, w):: (to_nod_f e (w::s) l)
end.

Definition to_nod (e : list E):= to_nod_f e nil (to_no_l e).
(*无重复边的匹配M*)
Definition Mn := to_nod M.

Theorem edge_eq : forall (e1 e2 : E), e1 = e2 <-> fst e1 = fst e2 /\ snd e1 = snd e2.
Proof.
  intros.
  destruct e1.  simpl.
  split. intros. subst. simpl. auto.
  intros. destruct H. destruct e2. simpl in H. simpl in H0.
  subst. auto.
Qed.

Lemma one_edge_is_matching : forall e, In e (Elist BG) -> In (eo BG e) (Elist BG) ->is_matching (e::(eo BG e)::nil).
Proof.
  intros e X Y.
  unfold is_matching.
  split.
  - unfold disjoint_edges_set.
    intros.
    pose proof classic (e1 = e); pose proof classic (e2 = e).
    ...
    + simpl in H0. destruct H0.
      ...
      rewrite H3 in H2.
      unfold eo in H2.
      unfold snd in H2 at 1; unfold fst in H2 at 2.
      destruct H2.
      destruct e.
      unfold fst, snd.
      auto.
    ...
    + simpl in H.
      ...
      destruct H4. auto.
      destruct H0.
      rewrite H in H0. rewrite H0 in H1. destruct H1; auto.
      destruct H0.
      destruct H.
  - intros.
    split.
    ...
    simpl.
    left.
    unfold eo; unfold eo in H.
    destruct e, e0.
    unfold snd, fst; unfold snd, fst in H.
    apply edge_eq in H; rewrite edge_eq.
    unfold snd, fst; unfold snd, fst in H.
    tauto.
    destruct H.
    simpl in H.
...
Lemma greater_NoDup : forall (a: E) (N: list E), NoDup (a::N) -> NoDup N.
Proof.
  intros.
  pose proof NoDup_cons_1.
  specialize (H0 E a N).
  apply H0 in H.
  exact H.
Qed.

Lemma greater_pre : forall N, is_matching  N ->
    let n := to_nod N in
     length Mn < (length n) -> 
   (**
      length Mn < length m -> exists p : list E, aug_path p
   *)
    exists p : list E, aug_path p.
Proof.
  intros.
  induction N; intros.
  - inversion H.
  ...
  - apply IHN.
    admit.
    pose proof classic (length Mn < length (to_nod N)).
    destruct H1.
    exact H1.
    apply not_lt in H1.
    assert (length (to_nod N) < length (to_nod (a::N))).
Admitted.

Lemma remove_e_In_p : forall e a p, In e (remove_e a p) -> In e p.
Proof.
  intros.
  pose proof (remove_e_not_in e a p).
  tauto.
Qed.

Lemma exist_smaller_match : forall a m n, length Mn < length n -> is_matching m -> a :: n = to_nod m -> exists t, n = to_nod t /\ is_matching t.
Proof.
  intros.
  exists (remove_e (eo BG a) (remove_e a m)).
  split.
  + revert H1 H. revert n; induction (to_nod m); intros.
    inversion H1.
    inversion H1.
    admit.
  + admit.
...
Theorem greater_match_to_aug_path:
  forall  n m, 
     length Mn < length n -> 
           n= to_nod m -> ( is_matching m ->  exists p, aug_path p).
Proof.
  intros.  (* remember (to_nod m ) as n.  *)
  revert H0 H1. revert m.
  induction  n.
  - simpl in H. inversion H.
  - intros. simpl in H.  inversion H.
    eapply greater_pre. apply H1.   rewrite <- H0. simpl. auto.
    assert (length Mn < length n).
    auto. pose proof (IHn H4).
    pose proof (exist_smaller_match a m n H4 H1 H0).
